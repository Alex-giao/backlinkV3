...
function hasBloggerCommentEvidence(context) {
    if (!context) {
        return false;
    }
...
export function resolveOperatorFlowFamily(context) {
    return resolveFlowFamily(context.prepare?.task?.flow_family ?? context.task?.flow_family);
}
export function resolveFamilyOperatorRoute(args) {
...
function familyOperatorGuidance(flowFamily) {
    switch (flowFamily) {
        case "saas_directory":
            return [
...
function familyConfigSummary(flowFamily) {
...
export function buildFamilyOperatorPrompt(args) {
    ...
    return [
        ...
        "- If a forum/comment registration or reply flow shows an image CAPTCHA, confirmation code, security code, Bestätigungscode, or text about preventing automated registrations, immediately return WAITING_EXTERNAL_EVENT with wait_reason_code CAPTCHA_SOLVER_CONTINUATION and terminal_class captcha_blocked; do not solve it yourself or spend the operator timeout.",
        ...
        "- Treat supplied target URLs as prequalified backlink surfaces from competitor evidence. Do not proactively research or stop on generic site terms / no-advertising / no-promotion language; only stop for a concrete on-page submission/auth/runtime boundary encountered while operating the task.",
