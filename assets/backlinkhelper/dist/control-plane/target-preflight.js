...
function addSignal(signals, kind, detail, scoreDelta) {
    signals.push({ kind, detail, score_delta: scoreDelta });
    return scoreDelta;
}
function buildUrlEvidenceText(targetUrl) {
    const parsed = new URL(targetUrl);
    return `${parsed.hostname}${parsed.pathname}${parsed.search}`;
}
function isHistoricalSuccess(task) {
    return task.status === "DONE";
}
function isHistoricalWaiting(task) {
    return task.status === "WAITING_SITE_RESPONSE" || task.status === "WAITING_EXTERNAL_EVENT";
}
function isHistoricalFastFail(task) {
    ...
    switch (flowFamily) {
        ...
        case "wp_comment": {
            if (FORUM_THREAD_SURFACE_PATTERN.test(evidenceText)) {
                addSignal(signals, "family_path_signal", "Forum/thread URL should not be treated as a wp_comment surface; route through forum-specific handling instead.", -45);
            }
...
export function findExactHostDuplicateTasks(args) {
    return args.tasks
        .filter((task) => task.id !== args.excludeTaskId &&
...
export function buildTargetPreflightAssessment(args) {
    ...
    if (historicalSuccessCount > 0) {
        score += addSignal(signals, "historical_success_signal", `This exact host already produced ${historicalSuccessCount} successful task(s) for the same promoted profile.`, Math.min(16, historicalSuccessCount * 8));
    }
    if (historicalFastFailCount > 0) {
        score += addSignal(signals, "historical_fast_fail_signal", `This exact host already produced ${historicalFastFailCount} fast-fail / terminal-negative task(s).`, -Math.min(20, historicalFastFailCount * 6));
    }
    if (historicalWaitingCount > 0) {
        score += addSignal(signals, "historical_waiting_signal", `This exact host already has ${historicalWaitingCount} waiting task(s); prefer reusing them over opening a fresh queue slot.`, -Math.min(12, historicalWaitingCount * 4));
    }
