...
import { buildRetryDecisionPlan, canRetry, deriveFlowFamilyAudit, getRetryReadyAt, deriveTaskLane, isRetryExhausted, matchesTaskScope, pickNextTaskForLane, parkExhaustedRetryableTask, recoverActiveWorkerLeaseAfterOperatorFailure, resolveWorkerLeaseGroupForLane, shouldDeferHostReactivation, shouldReactivateRuntimeRetries, } from "./task-queue.js";
import { __setActiveDataStoreForTest, loadTask, loadWorkerLease, saveTask, saveWorkerLease, } from "../memory/data-store.js";
...
function createInMemoryDataStore() {
    ...
    const targetSites = new Map();
    return {
        kind: "file",
        ensureDataDirectories: async () => { },
        loadTask: async (taskId) => (tasks.has(taskId) ? cloneJson(tasks.get(taskId)) : undefined),
        saveTask: async (task) => {
            tasks.set(task.id, cloneJson(task));
        },
        listTasks: async () => [...tasks.values()].map((task) => cloneJson(task)),
        loadWorkerLease: async (group = "active") => (leases.has(group) ? cloneJson(leases.get(group)) : undefined),
        loadAllWorkerLeases: async () => ({
            active: leases.has("active") ? cloneJson(leases.get("active")) : undefined,
            follow_up: leases.has("follow_up") ? cloneJson(leases.get("follow_up")) : undefined,
        }),
        saveWorkerLease: async (lease, group = lease.group ?? "active") => {
            leases.set(group, cloneJson({ ...lease, group }));
        },
        clearWorkerLease: async (group = "active") => {
            leases.delete(group);
        },
        clearWorkerLeaseForTask: async (taskId) => {
            let cleared = false;
        ...
        loadAccountRecord: async (hostname) => (accounts.has(hostname) ? cloneJson(accounts.get(hostname)) : undefined),
        saveAccountRecord: async (account) => {
            accounts.set(account.hostname, cloneJson(account));
        },
        loadCredentialRecord: async (credentialRef) => (credentials.has(credentialRef) ? cloneJson(credentials.get(credentialRef)) : undefined),
        saveCredentialRecord: async (record) => {
            credentials.set(record.credential_ref, cloneJson(record));
        },
        deleteCredentialRecord: async (credentialRef) => {
            credentials.delete(credentialRef);
        },
        upsertTargetSite: async (target) => {
...
function makeWorkerLease(taskId, overrides = {}) {
    return {
        task_id: taskId,
        owner: "test-worker",
...
test("pickNextTaskForLane prioritizes higher queue_priority_score before FIFO among READY active tasks", () => {
    ...
    assert.equal(pickNextTaskForLane([olderLowPriority, newerHighPriority], "directory_active")?.id, "newer-high-priority");
});
