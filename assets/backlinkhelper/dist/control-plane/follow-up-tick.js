...
import { clearWorkerLeaseForTask, ensureDataDirectories, getArtifactFilePath, saveTask, writeJsonFile, } from "../memory/data-store.js";
...
function matchesTaskHostname(rawUrl, taskHostname) {
...
function resolveFollowUpMailboxInputs(task, account) {
    const primaryEmail = account?.email ?? task.submission.submitter_email;
    const emailAlias = account?.email_alias ??
...
function resolveContinuationUrlCandidates(args) {
    ...
    return candidates.filter((candidate) => matchesTaskHostname(candidate, args.task.hostname));
}
...
function resolveLightweightSiteResponseUrl(task) {
...
function readAnchorAttribute(attributes, name) {
    const pattern = new RegExp(`${name}\\s*=\\s*(?:"([^"]*)"|'([^']*)'|([^\\s>]+))`, "i");
    const match = attributes.match(pattern);
    const value = match?.[1] ?? match?.[2] ?? match?.[3];
    return value ? decodeHtmlEntities(value.trim()) : undefined;
}
function hasBooleanAnchorAttribute(attributes, name) {
    const pattern = new RegExp(`(?:^|\\s)${name}(?:\\s|=|$)`, "i");
    return pattern.test(attributes);
}
function inferStaticAnchorVisibility(attributes) {
...
async function fetchLightweightPageHtml(url) {
...
export async function evaluateLightweightFollowUp(args) {
    ...
    if (previousStatus === "WAITING_EXTERNAL_EVENT" &&
        ...
        if (topCandidate?.verification_code) {
            ...
            const detail = `Verification email yielded a code-only continuation for ${args.task.hostname}; returning the task to the active lane.`;
            return {
                action: "activate_ready",
                detail,
    ...
    if (previousStatus === "WAITING_SITE_RESPONSE" &&
...
export function applyLightweightFollowUpAction(args) {
    ...
    if (args.evaluation.action === "activate_ready") {
        ...
        task.email_verification_continuation = undefined;
        task.wait = args.lease.previous_wait;
...
export async function runFollowUpTick(args) {
    ...
    return {
        mode: evaluation.action === "activate_ready"
            ? "activated_ready"
