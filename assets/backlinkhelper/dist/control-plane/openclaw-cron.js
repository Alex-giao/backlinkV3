...
function getManagedDescription(args) {
    return `${MANAGED_DESCRIPTION_PREFIX} | repo=${REPO_ROOT} | cdp=${args.cdpUrl}`;
}
function buildCronMessage(args) {
    return [
...
function findManagedJob(jobs, name) {
...
function buildCommonCronArgs(args, description, message) {
    const cliArgs = [
        "--name",
        args.name,
        "--description",
...
export async function ensureOpenClawCron(args) {
    const description = getManagedDescription(args);
    const message = buildCronMessage(args);
    const listResponse = parseJsonFromOutput(runOpenClaw(["cron", "list", "--all", "--json"]).stdout);
    ...
    const created = parseJsonFromOutput(addOutput.stdout);
    return {
        action: "add",
