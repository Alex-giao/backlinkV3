import { loadRuntimeIncident } from "./runtime-incident.js";
import { loadRuntimeRecoveryStatus } from "./runtime-sanitize.js";
...
export async function inspectBrowserTargetHealth(cdpUrl) {
...
export async function probeRuntimeHealth(cdpUrl, options = {}) {
    try {
        ...
        if (recoveryStatus?.last_attempt) {
            summaryParts.push(`last recovery ${recoveryStatus.last_attempt.recovered ? "recovered" : "failed"}: ${recoveryStatus.last_attempt.detail}`);
        }
        return {
            healthy,
