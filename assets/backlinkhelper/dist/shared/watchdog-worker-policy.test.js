...
import { acquireWorkerLock, classifyCampaignRun, refreshWorkerLock, releaseWorkerLock, shouldStopWorkerLoop, } from "./watchdog-worker-policy.js";
function tmpLockDir() {
...
test("classifyCampaignRun separates productive task runs from idle and hard-stop outcomes", () => {
    assert.deepEqual(classifyCampaignRun({ stop_reason: "max_active_tasks", active_tasks_started: 1, active_tasks_finalized: 1 }), { productive: true, idle: false, hard_stop: false, reason: "max_active_tasks" });
    assert.deepEqual(classifyCampaignRun({ stop_reason: "no_candidate", active_tasks_started: 0, active_tasks_finalized: 0 }), { productive: false, idle: true, hard_stop: false, reason: "no_candidate" });
    assert.deepEqual(classifyCampaignRun({ stop_reason: "scope_mismatch", active_tasks_started: 0, active_tasks_finalized: 0 }), { productive: false, idle: false, hard_stop: true, reason: "scope_mismatch" });
});
test("shouldStopWorkerLoop stops before starting unsafe extra work", () => {
    assert.equal(shouldStopWorkerLoop({
        iteration: 10,
        maxIterations: 10,
        idleCount: 0,
        idleLimit: 2,
