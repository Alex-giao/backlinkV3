...
import { buildCommentCopyPrompt, chooseFallbackCommentCopy, extractRecentCopyHistory, validateCommentCopyPlan, } from "./comment-copy-policy.js";
...
test("validateCommentCopyPlan rejects repeated recent anchor and malformed link counts", () => {
    ...
    assert.ok(repeated.errors.some((error) => error.includes("recent anchor")));
    const tooManyLinks = validateCommentCopyPlan({
        anchor_text: "quick fruit puzzle",
        comment_html: 'Nice post. <a href="https://suikagame.fun/">quick fruit puzzle</a> and <a href="https://example.com/">another link</a>.',
        reason: "contextual",
    }, {
        promotedUrl: "https://suikagame.fun/",
        recentAnchors: [],
    ...
    assert.ok(tooManyLinks.errors.some((error) => error.includes("exactly one link")));
});
test("validateCommentCopyPlan rejects non-Latin anchors on English-context submissions", () => {
    ...
    assert.ok(result.errors.some((error) => error.includes("Latin")));
});
...
test("extractRecentCopyHistory returns promoted DONE anchors newest first", () => {
    ...
    assert.deepEqual(history.map((item) => item.anchor_text), ["simple browser puzzle", "Suika Game"]);
});
