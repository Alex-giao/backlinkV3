...
test("extractMissingInputFields normalizes common field aliases from free text", () => {
    ...
    assert.deepEqual(fields.map((field) => field.key), ["postal_code", "address_line_1", "city", "phone_number"]);
    assert.deepEqual(fields.map((field) => ({ key: field.key, label: field.label, field_class: field.field_class })), [
...
test("summarizeMissingInputPreflight separates dossier-backed resolved fields, auto-resolvable profile fields, and unresolved fields", () => {
    ...
    assert.deepEqual(report.resolved_fields.map((field) => ({ key: field.key, value: field.value, source: field.source })), [{ key: "phone_number", value: "+1-307-555-0100", source: "dossier.phone_number" }]);
    assert.deepEqual(report.auto_resolvable_fields.map((field) => ({ key: field.key, value: field.value })), [
        { key: "country", value: "United States" },
        { key: "linkedin_url", value: "https://linkedin.com/company/exactstatement" },
    ]);
    assert.deepEqual(report.unresolved_fields.map((field) => ({ key: field.key, count: field.count })), [
...
test("summarizeMissingInputPreflight prefers structured missing_fields when present", () => {
    ...
    assert.deepEqual(report.resolved_fields.map((field) => field.key), []);
    assert.deepEqual(report.auto_resolvable_fields.map((field) => field.key), ["primary_category"]);
    assert.deepEqual(report.unresolved_fields.map((field) => field.key), ["logo_url"]);
    assert.equal(report.completeness.core_ready, true);
});
