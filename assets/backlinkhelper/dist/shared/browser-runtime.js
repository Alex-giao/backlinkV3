...
function hostPreference(cdpUrl) {
    try {
...
function scoreExternalCandidate(metadata) {
    const userAgent = metadata.user_agent ?? "";
    return userAgent.includes("HeadlessChrome") ? 0 : 10;
}
async function autodiscoverExternalCdpUrl() {
...
export async function resolveCdpUrl(cliValue) {
    if (cliValue) {
        return {
...
async function fetchBrowserMetadata(cdpUrl) {
    if (!cdpUrl.startsWith("http://") && !cdpUrl.startsWith("https://")) {
        return {
...
export async function resolveBrowserRuntime(cliCdpUrl) {
