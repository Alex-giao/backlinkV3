...
function getHermesHomePath(override) {
    return override ?? process.env.HERMES_HOME ?? path.join(os.homedir(), ".hermes");
}
function getGoogleWorkspaceApiPath(hermesHome) {
    return path.join(getHermesHomePath(hermesHome), "skills", "productivity", "google-workspace", "scripts", "google_api.py");
}
function getGoogleWorkspaceSetupPath(hermesHome) {
    return path.join(getHermesHomePath(hermesHome), "skills", "productivity", "google-workspace", "scripts", "setup.py");
}
function getPreferredPythonBin(hermesHome) {
...
function formatMailbox(value) {
    if (!value?.email) {
        return undefined;
    }
    return value.name ? `${value.name} <${value.email}>` : value.email;
}
export function parseGwsSearchPayload(raw) {
...
export function parseGwsGetPayload(raw) {
    const parsed = JSON.parse(raw);
    return {
...
function ensureUnreadQuery(query) {
    return /\bis:unread\b/i.test(query) ? query : `is:unread ${query}`.trim();
}
function normalizeText(value) {
    return (value ?? "").toLowerCase();
}
function normalizeHostnameTokens(hostname) {
    if (!hostname) {
        return [];
    }
...
async function searchEmailsWithProvider(args) {
    ...
    if (args.provider === "gog") {
        const result = await runner("gog", buildGogArgs({
            query: args.query,
    ...
    const result = await runner(pythonBin, [googleApiPath, "gmail", "search", args.query, "--max", String(args.max ?? 1)], 30_000);
    if (result.exit_code !== 0 || !result.stdout.trim()) {
        return [];
    }
...
async function getEmailBodyWithProvider(args) {
    ...
    const pythonBin = args.pythonBin ?? getPreferredPythonBin(args.hermesHome);
    const googleApiPath = getGoogleWorkspaceApiPath(args.hermesHome);
    const result = await runner(pythonBin, [googleApiPath, "gmail", "get", args.messageId], 30_000);
    if (result.exit_code !== 0 || !result.stdout.trim()) {
        return undefined;
    }
...
export async function triageRecentUnreadEmails(args) {
    ...
    const scored = [...deduped.values()].map((message) => scoreCandidate({
        message,
...
export function extractMagicLink(body) {
