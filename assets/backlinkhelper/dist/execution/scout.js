...
function extractHints(text, familyConfig) {
    ...
    const evidence_sufficiency = familyConfig.scout.evidenceSignals.some((signal) => normalized.includes(signal)) || field_hints.length > 0;
    return {
        field_hints,
        auth_hints,
        anti_bot_hints,
...
async function collectVisibleSurfaceText(root) {
    return root
        ...
        for (const element of elements) {
            ...
            const safePlaceholder = normalizedPlaceholder.includes("@") || /^https?:\/\//i.test(normalizedPlaceholder)
                ? undefined
...
async function collectActionCandidates(root) {
    const rawCandidates = await root
        ...
        for (const element of elements) {
            ...
            const area = areaTag === "header" || areaTag === "nav"
                ? "header"
                : areaTag === "footer"
                    ? "footer"
                    : areaTag === "main"
            ...
            rows.push({
                ...
                text: (htmlElement.innerText || element.value || "").replace(/\s+/g, " ").trim(),
                href,
...
async function collectEmbedHints(args) {
    ...
    for (let index = 0; index < Math.min(count, 8); index += 1) {
        ...
        const likelyInteractive = provider !== "recaptcha" &&
...
export async function runLightweightScout(args) {
    ...
    try {
        ...
        const message = error instanceof PlaywrightSessionTimeoutError
