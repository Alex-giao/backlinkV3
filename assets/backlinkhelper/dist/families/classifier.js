...
const EMBEDDED_FORUM_THREAD_ROUTE_PATTERN = /(?:^|\/)\b(?:viewtopic|showthread)\.php(?:[?#/]|$)|(?:^|\/)\b(?:d|t)\/\d+\/[^?#\s]+\/(?:discussion|discussions|discussione)\.aspx\b|(?:^|\/)\b(?:discussion|discussions|thread|threads|topic|topics)\/(?:view\/)?\d+(?:[-/.]|$)/i;
...
function isReservedForumRouteSegment(segment) {
    return RESERVED_FORUM_ROUTE_SEGMENTS.has(segment);
}
function segmentAfter(segments, segmentNames) {
...
function hasConcreteContentRouteAfter(segments, segmentNames) {
    for (let index = 0; index < segments.length - 1; index += 1) {
        if (!segmentNames.has(segments[index])) {
            continue;
        }
...
function hasForumPostViewRoute(segments) {
...
function hasEmbeddedForumThreadRoute(parsed) {
...
function isForumSlugLike(segment) {
...
function hasNestedForumSlugRoute(segments) {
    for (let index = 0; index < segments.length - 2; index += 1) {
...
export function isForumThreadTargetUrl(targetUrl) {
...
export function isBlogCommentArticleTargetUrl(targetUrl) {
    const parsed = parseTargetUrl(targetUrl);
    if (!parsed) {
        return false;
    }
...
export function inferTargetFlowFamily(targetUrl) {
    if (isForumThreadTargetUrl(targetUrl)) {
        return "forum_post";
    }
...
function describeInferredSurface(flowFamily) {
...
export function applyTargetFlowFamilyClassificationToTask(args) {
    ...
    const shouldApply = classification.flowFamily !== existingFamily ||
