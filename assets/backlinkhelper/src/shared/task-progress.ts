...
import type {
  ...
  TaskEvidence,
  TaskExecutionState,
  TaskInteractionFrontier,
  TaskRecord,
  TaskReusableFragment,
  TaskStatus,
  TerminalClass,
...
function ensureExecutionState(task: TaskRecord): TaskExecutionState {
...
function inferConfidence(value?: number, fallback: PageClassificationConfidence = "medium"): PageClassificationConfidence {
...
function signalMatches(text: string, patterns: RegExp[]): boolean {
  return patterns.some((pattern) => pattern.test(text));
}

function toSignalPatterns(values: readonly string[]): RegExp[] {
...
function inferContextType(args: {
  ...
  const hasCorroboratedBoundarySignal = (signals: readonly string[]) =>
    boundaryCorpus.length > 0 && signalMatches(corpus, toSignalPatterns(signals)) && signalMatches(boundaryCorpus, toSignalPatterns(signals));
...
function inferSignals(args: {
  ...
  if (
    args.visual?.classification === "success_or_confirmation" ||
    args.taskStatus === "WAITING_SITE_RESPONSE" ||
...
function maybeUpsertEvidence(state: TaskExecutionState, evidence: TaskEvidence): void {
  upsertById(state.evidence, "evidence_id", evidence, MAX_EVIDENCE);
}

function maybeUpsertAction(state: TaskExecutionState, action: TaskDiscoveredAction): void {
  upsertById(state.discovered_actions, "action_id", action, MAX_ACTIONS);
}

function maybeUpsertFragment(state: TaskExecutionState, fragment: TaskReusableFragment): void {
  upsertById(state.reusable_fragments, "fragment_id", fragment, MAX_FRAGMENTS);
}

function replaceActiveBlockers(state: TaskExecutionState, blockers: TaskBlocker[]): void {
  const resolvedAt = new Date().toISOString();
  state.blockers = state.blockers.map((blocker) =>
...
function inferBlockersFromOutcome(args: {
  ...
  const buildBlocker = (blockerType: string, options: {
    severity: "soft" | "hard";
...
function inferFragments(args: {
  ...
  submitCandidates?: string[];
  visual?: VisualVerificationResult;
  source: TaskReusableFragment["source"];
  proofRefs: string[];
  flowFamily?: TaskRecord["flow_family"];
}): TaskReusableFragment[] {
  const corpus = [args.title, args.text, ...(args.fieldHints ?? []), ...(args.authHints ?? []), ...(args.submitCandidates ?? [])]
    ...
    .join(" ")
    .toLowerCase();
  const fragments: TaskReusableFragment[] = [];
  ...
  if (
    signalMatches(corpus, toSignalPatterns(taskProgress.formSignals)) ||
...
export function updateTaskExecutionStateFromTrace(args: {
  ...
  args.trace.steps.forEach((rawStep, index) => {
    ...
    const toNodeId = stableId(
      "node",
      nextObservation?.url ?? args.handoff.current_url ?? args.trace.final_url,
      nextObservation?.title ?? args.trace.final_title,
      toContextType,
    );
    ...
    maybeUpsertAction(state, {
      ...
      label,
      selector_hint:
        typeof step.decision.index === "number"
          ? `index:${step.decision.index}`
  ...
  const finalNodeId = stableId(
    "node",
    args.handoff.current_url || args.trace.final_url,
    args.trace.final_title,
    finalContextType,
  );
...
export function updateTaskExecutionStateFromOutcome(args: {
  ...
  replaceActiveBlockers(
    state,
    inferBlockersFromOutcome({
      nodeId,
      contextType,
      url: args.currentUrl ?? args.task.target_url,
      title: args.currentTitle,
