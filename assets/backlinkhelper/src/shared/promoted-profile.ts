...
import type { PromotedProfile, PromotedProfileFieldValue, PromotedProfileFieldSourceType, PromotedProfileSocialLinks } from "./types.js";
...
interface ProbePage {
  ...
  lines: string[];
  listItems: string[];
  socialLinks: PromotedProfileSocialLinks;
  emails: string[];
  html: string;
}
...
function extractStructuredTexts(html: string, tagName: string): string[] {
...
function cleanCandidateLine(line: string): string | undefined {
...
function isSameSiteUrl(candidateUrl: URL, baseUrl: URL): boolean {
  return normalizeHost(candidateUrl.hostname) === normalizeHost(baseUrl.hostname);
}

function extractSocialLinks(html: string, baseUrl: URL): PromotedProfileSocialLinks {
  const socialLinks: PromotedProfileSocialLinks = {};
...
function buildShortDescriptionCandidates(pages: ProbePage[]): string[] {
...
function pickTagline(pages: ProbePage[], fallbackDescription: string): string | undefined {
...
function pickLongDescription(pages: ProbePage[], shortDescription: string): string | undefined {
...
function pickFeatureBullets(pages: ProbePage[]): string[] {
...
function pickPricingSummary(pages: ProbePage[]): string | undefined {
  const pricingPages = pages.filter((page) => page.kind === "pricing");
  const pricingSignals = uniqStrings(
    pricingPages.flatMap((page) =>
      page.lines.filter((line) =>
        /(pricing|price|plan|free|trial|monthly|annual|yearly|contact sales|\$|usd)/i.test(line),
      ),
    ),
  );
...
function pickCompanyName(name: string, pages: ProbePage[]): string | undefined {
...
function pickContactEmail(pages: ProbePage[], hostname: string): string | undefined {
...
function mergeSocialLinks(pages: ProbePage[]): PromotedProfileSocialLinks | undefined {
  const merged: PromotedProfileSocialLinks = {};
  for (const page of pages) {
...
function scoreCandidateUrl(url: URL, anchorText: string): number {
  ...
  const keywordScores: Array<[number, string[]]> = [
...
function classifyProbePageKind(url: URL, anchorText: string): string {
  const combined = `${url.pathname} ${anchorText}`.toLowerCase();
...
function buildCandidatePages(homeUrl: string, html: string): Array<{ url: string; kind: string }> {
  const baseUrl = new URL(homeUrl);
  const linkedCandidates = new Map<string, { score: number; kind: string }>();
  ...
  const defaultPaths: Array<{ path: string; kind: string; score: number }> = [
  ...
  const fallbackCandidates: Array<{ url: string; kind: string; score: number }> = [];
...
async function fetchSiteHtml(url: string): Promise<{ url: string; html: string }> {
...
async function probePage(url: string, kind: string): Promise<ProbePage | undefined> {
  try {
    ...
    return {
      ...
      siteName:
        extractMetaContent(fetched.html, "og:site_name") ??
...
function buildDossierField(args: {
  ...
  confidence?: PromotedProfileFieldValue["confidence"];
  allowedForAutofill?: boolean;
}): PromotedProfileFieldValue | undefined {
  if (!args.value?.trim()) {
    return undefined;
  }
...
function buildDossierFields(profile: PromotedProfile): Record<string, PromotedProfileFieldValue> | undefined {
  ...
  ].filter((field): field is PromotedProfileFieldValue => Boolean(field));
...
export function applyDossierUpdates(args: {
  ...
  const nextFields: Record<string, PromotedProfileFieldValue> = {
...
function applyProfileOverrides(profile: PromotedProfile, args: {
  promotedName?: string;
  promotedDescription?: string;
}): PromotedProfile {
  const withOverrides: PromotedProfile = {
    ...profile,
...
function shouldReuseExistingProfile(existing: PromotedProfile | undefined, promotedUrl: string): boolean {
  return Boolean(
    existing &&
...
export async function probePromotedProfile(args: {
  ...
  promotedName?: string;
  promotedDescription?: string;
  existingProfile?: PromotedProfile;
  ...
  const supplementalPages: ProbePage[] = [];
...
export async function loadOrCreatePromotedProfile(args: {
  ...
  if (
    ...
    return existing as PromotedProfile;
  }
