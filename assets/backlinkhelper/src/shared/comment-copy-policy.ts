...
export interface CommentCopyPlan {
  anchor_text: string;
  comment_html: string;
  reason?: string;
  source?: "ai" | "fallback";
...
export interface CommentCopyHistoryItem {
  task_id: string;
  anchor_text?: string;
...
export interface CommentCopyPromptArgs {
  promotedUrl: string;
  promotedName: string;
  promotedDescription?: string;
  pageTitle: string;
  pageExcerpt: string;
  recentHistory?: CommentCopyHistoryItem[];
}

export interface CommentCopyValidationArgs {
  promotedUrl: string;
...
export interface CommentCopyValidationResult {
  ok: boolean;
  errors: string[];
  anchor_text?: string;
  link_count: number;
...
export function normalizeUrlForCopyPolicy(rawUrl: string): string {
  try {
...
function taskPromotedUrl(task: TaskRecord): string | undefined {
  return task.submission?.promoted_profile?.url;
}

function samePromotedUrl(a: string | undefined, b: string): boolean {
  if (!a) {
    return false;
  }
...
export function extractRecentCopyHistory(
  tasks: Pick<
    TaskRecord,
    "id" | "status" | "updated_at" | "submission" | "link_verification" | "latest_artifacts"
  >[],
  promotedUrl: string,
  limit = 8,
): CommentCopyHistoryItem[] {
  return tasks
    .filter((task) => task.status === "DONE" && samePromotedUrl(taskPromotedUrl(task as TaskRecord), promotedUrl))
...
export function validateCommentCopyPlan(
  plan: CommentCopyPlan,
  args: CommentCopyValidationArgs,
): CommentCopyValidationResult {
  const errors: string[] = [];
...
function chooseAnchor(recentAnchors: string[], seed: string): string {
...
function cleanContextSnippet(value: string, max = 120): string {
...
export function buildCommentCopyPrompt(args: CommentCopyPromptArgs): string {
