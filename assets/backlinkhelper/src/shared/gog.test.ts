...
import {
  detectMailProvider,
  parseGwsGetPayload,
  parseGwsSearchPayload,
} from "./gog.js";
import type { MailCommandRunner } from "./gog.js";

function commandResult(args: { stdout?: string; stderr?: string; exit_code?: number } = {}) {
  return {
    stdout: args.stdout ?? "",
    stderr: args.stderr ?? "",
...
test("detectMailProvider falls back to gws when gog is unavailable but google-workspace is authenticated", async () => {
  ...
  const runner: MailCommandRunner = async (command, args) => {
  ...
  assert.equal(
    calls.some((call) => call.command === "python3" && call.args.includes(setupScript)),
    true,
  );
});

test("parseGwsSearchPayload converts wrapper gmail search output into message summaries", () => {
  const messages = parseGwsSearchPayload(
    JSON.stringify({
      messages: [
        {
          id: "abc123",
...
test("parseGwsGetPayload converts wrapper gmail get output into normalized body fields", () => {
  const payload = parseGwsGetPayload(
    JSON.stringify({
