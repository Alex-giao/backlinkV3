...
import type {
  MissingInputAskPriority,
  MissingInputField,
  MissingInputFieldClass,
  MissingInputFieldConfidence,
  MissingInputFieldSummary,
  MissingInputPreflightReport,
  MissingInputRecommendedResolution,
  MissingInputResolvedField,
  PromotedProfile,
  TaskRecord,
} from "./types.js";

export interface MissingInputFieldDefinition {
  key: string;
  label: string;
...
const MISSING_INPUT_FIELD_DEFINITIONS: MissingInputFieldDefinition[] = [
  {
    key: "postal_code",
    label: "Postal Code",
...
function normalizeStructuredField(field: MissingInputField): MissingInputField {
...
function uniqueFields(fields: MissingInputField[]): MissingInputField[] {
  const seen = new Set<string>();
  const result: MissingInputField[] = [];
...
function findFieldDefinition(key: string): MissingInputFieldDefinition | undefined {
  return MISSING_INPUT_FIELD_DEFINITIONS.find((definition) => definition.key === key);
}

function findFieldDefinitionForText(text: string): MissingInputFieldDefinition | undefined {
  const normalized = text.trim();
  if (!normalized) {
    return undefined;
  }

  return MISSING_INPUT_FIELD_DEFINITIONS.find(
    (definition) => definition.key === normalized.toLowerCase() || definition.patterns.some((pattern) => pattern.test(normalized)),
  );
}

export function lookupMissingInputFieldDefinition(keyOrLabel: string): MissingInputFieldDefinition | undefined {
  return findFieldDefinition(keyOrLabel.trim().toLowerCase()) ?? findFieldDefinitionForText(keyOrLabel);
}

function defaultParsedConfidence(): MissingInputFieldConfidence {
  return "medium";
}

function definitionToMissingField(
  definition: MissingInputFieldDefinition,
  overrides: Partial<MissingInputField> = {},
): MissingInputField {
  return {
    key: definition.key,
    label: definition.label,
...
function labelizeCategoryHint(value: string): string {
  return value
    .split(/[-_\s]+/)
    .filter(Boolean)
...
function resolveKnownProfileValue(
  profile: PromotedProfile,
  key: string,
): { field: MissingInputResolvedField; mode: "resolved" | "auto_resolvable" } | undefined {
...
export function extractMissingInputFields(texts: Array<string | undefined>): MissingInputField[] {
  ...
  const detected = MISSING_INPUT_FIELD_DEFINITIONS.filter((definition) =>
    definition.patterns.some((pattern) => pattern.test(combined)),
  ).map((definition) =>
    definitionToMissingField(definition, {
...
function collectTaskMissingFields(task: TaskRecord): MissingInputField[] {
...
function isMissingInputTask(task: TaskRecord): boolean {
  return task.status === "WAITING_MISSING_INPUT" || task.wait?.wait_reason_code === "REQUIRED_INPUT_MISSING";
}

function resolvePreflightFamilyConfig(tasks: TaskRecord[]) {
...
function buildCompleteness(args: {
  ...
  tasks: TaskRecord[];
}): MissingInputPreflightReport["completeness"] {
...
export function summarizeMissingInputPreflight(args: {
  ...
  const fieldSummary = new Map<string, MissingInputFieldSummary>();
  ...
  const resolvedFields: MissingInputResolvedField[] = [];
  const autoResolvableFields: MissingInputResolvedField[] = [];
  const unresolvedFields: MissingInputFieldSummary[] = [];
  ...
  const sortByPriority = <T extends { key: string; label: string }>(left: T, right: T): number => {
...
export function enrichWaitMetadataWithMissingFields(task: TaskRecord): TaskRecord {
