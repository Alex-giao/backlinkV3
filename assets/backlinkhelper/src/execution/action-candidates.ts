import type { ScoutLinkCandidate } from "../shared/types.js";
...
interface RankedActionCandidate extends ScoutLinkCandidate {
  score: number;
}
...
function isSubmitControl(candidate: RawActionCandidate): boolean {
...
function hasIgnorableHref(candidate: RawActionCandidate): boolean {
...
function scoreCandidate(candidate: RawActionCandidate): number {
  ...
  if (top <= 320) score += 10;
  if (candidate.tag === "button" || candidate.tag === "input") score += 6;
...
export function buildScoutActionCandidates(rawCandidates: RawActionCandidate[]): {
  ...
  const ranked: RankedActionCandidate[] = [];
