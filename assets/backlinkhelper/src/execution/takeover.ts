...
function buildCaptchaSolverContinuationOutcome(evidenceRef: string, detail?: string): ProposedOutcome {
...
export function applyCaptchaSolverOutcomeGuard(args: {
  outcome: ProposedOutcome;
  captchaSolverAttempt?: CapsolverAttemptRecord;
  evidenceRef: string;
}): ProposedOutcome {
  if (
    args.captchaSolverAttempt?.submit_blocked !== true ||
  ...
  const missingFields = Array.isArray(args.captchaSolverAttempt.missing_fields)
    ? args.captchaSolverAttempt.missing_fields.filter((field): field is string => typeof field === "string" && field.trim().length > 0)
    : [];
  const fieldDetail = missingFields.length > 0 ? ` Missing account fields: ${missingFields.join(", ")}.` : "";
  return buildCaptchaSolverContinuationOutcome(
    args.evidenceRef,
    `${args.captchaSolverAttempt.detail}${fieldDetail} Keep the task resumable by the full active worker; do not submit a CAPTCHA-only empty registration form.`,
  );
}
...
function isAllowedGoogleOauthTransition(bodyText: string, currentUrl: string): boolean {
  ...
  return /\b(?:continue|sign[ -]?in|log[ -]?in|login|sign[ -]?up|register|join|use|connect)\s+(?:with|using|via)\s+google\b/.test(
    normalized,
  ) || /\bwith google\b/.test(normalized) || /\bgoogle account\b/.test(normalized);
}
...
function containsNormalizedSignal(normalized: string, needle: string): boolean {
...
export function classifyEarlyTerminalOutcome(args: {
  ...
  const containsAny = (needles: readonly string[]) => needles.some((needle) => containsNormalizedSignal(normalized, needle));
...
const VALID_TASK_STATUSES = new Set<TakeoverResult["next_status"]>([
  "READY",
  "RUNNING",
...
function isTaskStatus(value: unknown): value is TakeoverResult["next_status"] {
  return typeof value === "string" && VALID_TASK_STATUSES.has(value as TakeoverResult["next_status"]);
}

function normalizeProposedOutcome(proposed?: ProposedOutcome): ProposedOutcome | undefined {
  if (!proposed || typeof proposed !== "object") {
    return undefined;
  }

  const raw = proposed as ProposedOutcome & { status?: unknown; next_status?: unknown; detail?: unknown };
...
function isNavigableHttpUrl(value: string | undefined): boolean {
  if (!value) {
    return false;
  }
...
export function shouldRebindFinalizationPageContext(args: {
  ...
  const expectedHostname =
    normalizeComparableHostname(args.preferredUrl) ?? normalizeComparableHostname(args.taskHostname) ?? args.taskHostname;
  const actualHostname = normalizeComparableHostname(args.currentUrl);
...
async function rebindFinalizationPageContextIfNeeded(args: {
  ...
  if (
    !shouldRebindFinalizationPageContext({
