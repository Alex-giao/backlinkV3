...
interface VisualVerifyArgs {
  ...
  fieldHints?: string[];
  antiBotHints?: string[];
  linkCandidates?: ScoutLinkCandidate[];
  flowFamily?: FlowFamily;
}

interface VisualRecoveryHintArgs extends VisualVerifyArgs {
  goal: string;
...
type RecoveryTargetKind = NonNullable<VisualRecoveryHint["target_kind"]>;
...
function buildVisualSuccessSignals(flowFamily?: FlowFamily): string[] {
  const family = buildFamilyPromptContext(flowFamily);
  return [...new Set([...SUCCESS_SIGNALS, ...family.confirmationCues])];
}

function buildInlineCaptchaSignals(flowFamily?: FlowFamily): string[] {
  const family = buildFamilyPromptContext(flowFamily);
  return [...new Set([...INLINE_CAPTCHA_SIGNALS, ...family.captchaCues])];
}

function buildFamilyPromptLines(flowFamily?: FlowFamily): string[] {
  const family = buildFamilyPromptContext(flowFamily);
  return [
...
function buildSignalText(args: VisualVerifyArgs): string {
  return [
    args.pageUrl,
    args.pageTitle,
    args.bodyExcerpt,
...
export function buildVisualPrompt(args: VisualVerifyArgs, mode: "primary" | "boundary_recheck"): string {
  const instructions = [
  ...
  if (mode === "boundary_recheck") {
    instructions.push(
      "Re-check the screenshot carefully for boundary cases between submit_form, captcha_or_human_verification, and success_or_confirmation.",
      "When the page contains both a form and an inline CAPTCHA/security-code requirement, prefer captcha_or_human_verification over submit_form.",
      "When the page contains both a form and an inline error after submission, prefer submit_form or unknown, not success_or_confirmation.",
    );
  }
...
export function shouldRunBoundaryRecheck(
  args: VisualVerifyArgs,
...
async function invokeVisualClassification(args: {
  ...
  return {
    classification: CLASSIFICATIONS.includes(classification as (typeof CLASSIFICATIONS)[number])
      ? (classification as VisualVerificationResult["classification"])
      : "unknown",
...
async function invokeVisualRecoveryHint(args: {
  ...
  const parsed = JSON.parse(raw) as Record<string, unknown>;
  const targetKind = String(parsed.target_kind ?? "unknown") as RecoveryTargetKind;
  const validKinds: RecoveryTargetKind[] = ["submit", "login", "signup", "form", "other", "unknown"];
  return {
...
export function buildRecoveryPrompt(args: VisualRecoveryHintArgs): string {
  return [
...
export async function runVisualVerification(
  args: VisualVerifyArgs,
): Promise<VisualVerificationResult | undefined> {
...
export async function runVisualRecoveryHint(
  args: VisualRecoveryHintArgs,
): Promise<VisualRecoveryHint | undefined> {
  const apiKey = process.env[args.config.api_key_env]?.trim();
  if (!apiKey) {
    return undefined;
  }
