...
import {
  ...
  buildBrowserUseSessionName,
  chooseFinalOutcome,
  choosePreferredFinalizationPageStateSample,
  resolveFinalizationPreferredUrl,
  shouldRebindFinalizationPageContext,
  classifyEarlyTerminalOutcome,
...
function baseRetryableOutcome(): ProposedOutcome {
  return {
...
test("finalization sampling prefers visible overlay surfaces over generic body text", () => {
  ...
  const modalSample = {
    ...
    title: "Submit your tool",
    bodyText:
      "Sign in to continue Password Continue with Google Submit your tool Name Description Submit",
...
test("signup continuation guard keeps mixed login/signup surface retryable", () => {
  const guarded = applySignupContinuationGuard({
    ...
    bodyText:
      "Welcome back Sign in to get started Sign in with magic link Don't have an account yet? Sign up Username Email Password Password confirmation Create my account",
  });
...
test("early terminal classifier promotes clear required-field blockers to WAITING_MISSING_INPUT", () => {
  const classified = classifyEarlyTerminalOutcome({
    ...
    bodyText:
      "Please complete all required fields before submitting. Missing required fields: Phone Number, City, Postal Code.",
    evidenceRef: "artifact.json",
  });
  ...
  assert.deepEqual(
    classified.outcome.wait?.missing_fields?.map((field) => field.key),
...
test("early terminal classifier promotes reciprocal-backlink requirements to policy wait instead of retry", () => {
  const classified = classifyEarlyTerminalOutcome({
    ...
    title: "Add your site",
    bodyText:
      "To get listed, first add our backlink to your website and share the live reciprocal backlink URL for review.",
    evidenceRef: "artifact.json",
  });
...
test("early terminal classifier ignores generic sponsor or pricing copy without an explicit paid-listing boundary", () => {
  const classified = classifyEarlyTerminalOutcome({
    ...
    title: "Submit your tool",
    bodyText:
      "Free submit is available for everyone. Sponsored by Stripe. Optional newsletter subscription for readers. Advertiser analytics starts at $49 per month.",
    evidenceRef: "artifact.json",
  });
