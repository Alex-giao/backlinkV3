...
import type { AgentBackendConfig, AgentDecision, AgentDecisionInput, FlowFamily } from "../shared/types.js";
...
const COMMON_SYSTEM_PROMPT = [
  ...
  "- only on HTTPS signup/register pages or HTTPS account URLs; never enter registration credentials on http:// pages or forms",
  ...
  "When a login/comment gate exposes Register, Sign up, Create account, Join, language chooser, or same-brand sibling-domain account links, actively follow that registration path before declaring manual auth.",
  "If a registration link first opens a language/region selector, pick the language matching the current page or English when available, then retry the visible Register/Sign up action.",
...
export function buildSystemPrompt(flowFamily?: FlowFamily): string {
...
function sanitizeDecision(raw: AgentDecision): AgentDecision {
  const normalizedText =
    raw.action === "input_index" && typeof raw.text !== "string" && typeof raw.value === "string"
      ? raw.value
      : raw.text;
