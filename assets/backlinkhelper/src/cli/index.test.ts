...
test("single-task operator guard blocks mutating queue/finalization commands", () => {
  ...
  assert.throws(
    () => assertCommandAllowedInSingleTaskOperatorMode("claim-next-task", guardedEnv),
    /single-task family operators/,
  );
  assert.throws(
    () => assertCommandAllowedInSingleTaskOperatorMode("task-finalize", guardedEnv),
    /single-task family operators/,
  );
