...
test("wrangler D1 SQL interpolation escapes quote-heavy JSON values", () => {
  ...
  assert.equal(
    __testInterpolateParams("INSERT INTO t (a, b, c) VALUES (?, ?, ?)", ["O'Reilly", payload, 3]),
    `INSERT INTO t (a, b, c) VALUES ('O''Reilly', '${payload.replace(/'/g, "''")}', 3)`,
  );
...
test("wrangler D1 JSON parser accepts direct and nested result envelopes", () => {
  assert.deepEqual(
    __testParseWranglerJson(JSON.stringify([{ results: [{ id: "a" }], success: true }])),
    [{ id: "a" }],
  );
  assert.deepEqual(
    __testParseWranglerJson(JSON.stringify({ result: [{ results: [{ id: "b" }] }] })),
    [{ id: "b" }],
  );
});
