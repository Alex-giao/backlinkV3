...
export function __setRuntimeRecoveryHookForTest(
  hook: typeof tryAutoRecoverRuntimeIncident,
): void {
  __testTryAutoRecoverRuntimeIncident = hook;
}

export function __resetRuntimeRecoveryHookForTest(): void {
  __testTryAutoRecoverRuntimeIncident = tryAutoRecoverRuntimeIncident;
}

type RetryDecisionBucket =
  | "terminal_success"
  | "external_email_verification"
  | "external_captcha_solver"
  | "reactivation_cooldown"
  | "reactivate_ready"
  | "runtime_reactivate_ready"
  | "runtime_recovery_pool"
  | "terminal_manual_auth"
  | "terminal_policy"
  | "terminal_missing_input"
  | "community_strategy"
  | "needs_manual_triage";

interface RetryDecisionArtifactHints {
  ...
  bodyExcerpt?: string;
  title?: string;
  classifierHypothesis?: string;
  classifierRecommendedState?: TaskStatus;
  classifierBusinessOutcome?: string;
  classifierAllowRerun?: boolean;
  visualClassification?: string;
  visualConfidence?: number;
  visualSummary?: string;
}

interface RetryDecisionPlan {
  taskId: string;
  hostname: string;
  bucket: RetryDecisionBucket;
...
interface RepartitionRetryDecisionResult {
  inspected: number;
  changed: number;
  byBucket: Record<string, number>;
  plans: RetryDecisionPlan[];
}
...
export interface RetryRepartitionRuntimeHealth {
  healthy: boolean;
  summary: string;
}
...
function semanticContractIncludes(
  flowFamily: TaskRecord["flow_family"] | undefined,
  key: "pending_wait_reason_codes" | "progress_wait_reason_codes" | "review_wait_reason_codes" | "policy_wait_reason_codes",
  reason: string,
): boolean {
...
const DEFAULT_GUARDED_REACTIVATION_BUCKETS: RetryDecisionBucket[] = [
  "reactivate_ready",
  "runtime_reactivate_ready",
];
...
export function deriveFlowFamilyAudit(args: {
  ...
  flowFamily: NonNullable<TaskRecord["flow_family"]>;
  flowFamilySource: FlowFamilySource;
  flowFamilyReason: string;
  flowFamilyUpdatedAt: string;
  correctedFromFamily?: TaskRecord["flow_family"];
  enqueuedBy: string;
} {
    ...
    familyAudit =
      defaultedFamily === existingFamily
        ...
        : {
            ...
            flowFamilySource: classification.source === "inferred" ? "inferred" as const : "corrected" as const,
...
function parseBucketList(values?: string[]): RetryDecisionBucket[] | undefined {
  if (!values || values.length === 0) {
    return undefined;
  }
  const unique = [...new Set(values)] as RetryDecisionBucket[];
  return unique.length > 0 ? unique : undefined;
}

function buildScopedTaskList(tasks: TaskRecord[], scope: TaskScopeFilter = {}): TaskRecord[] {
  return tasks.filter((task) => matchesTaskScope(task, scope));
}

const FOLLOW_UP_STATUSES = new Set<TaskStatus>(["WAITING_SITE_RESPONSE", "WAITING_EXTERNAL_EVENT"]);
...
export function resolveWorkerLeaseGroupForLane(lane: ClaimLane = "active_any"): WorkerLeaseGroup {
...
export function pickNextTaskForLane(tasks: TaskRecord[], lane: ClaimLane = "active_any"): TaskRecord | undefined {
  if (lane === "follow_up") {
    const emailFollowUpTasks = tasks
      .filter(
        (task) =>
          matchesClaimLane(task, lane) &&
...
export function buildTaskLaneReport(tasks: TaskRecord[]): {
  totals: {
    directory_active: number;
    non_directory_active: number;
    follow_up: number;
    blocked_or_other: number;
  };
} {
  const totals = {
    directory_active: 0,
...
export function isRetryExhausted(task: Pick<TaskRecord, "status" | "run_count">): boolean {
  return task.status === "RETRYABLE" && task.run_count >= MAX_AUTOMATIC_RETRIES + 1;
}

export function canRetry(task: TaskRecord): boolean {
...
function getRetryBackoffMs(task: Pick<TaskRecord, "wait">): number {
...
export function getRetryReadyAt(task: Pick<TaskRecord, "updated_at" | "wait">): string {
  return new Date(new Date(task.updated_at).getTime() + getRetryBackoffMs(task)).toISOString();
}

function inferExhaustedRetryReason(task: TaskRecord): string | undefined {
  const prioritizedWaitReason =
    task.wait?.wait_reason_code && !GENERIC_WAIT_REASON_CODES.has(task.wait.wait_reason_code)
  ...
  return inferReasonFromText(
    [task.last_takeover_outcome, task.wait?.resume_trigger, ...task.notes.slice(-5)]
      .filter(Boolean)
      .join("\n"),
    task.flow_family,
  );
...
export function parkExhaustedRetryableTask(task: TaskRecord): boolean {
  if (!isRetryExhausted(task)) {
    return false;
  }
  ...
  const exhaustedDetail =
    `Automatic retry budget exhausted after ${task.run_count} run(s). ` +
...
function matchesCommunityHost(hostname: string): boolean {
  return COMMUNITY_HOST_PATTERNS.some((pattern) => hostname === pattern || hostname.endsWith(`.${pattern}`));
}

export function shouldReactivateRuntimeRetries(
  runtimeHealth?: RetryRepartitionRuntimeHealth,
): boolean {
  return Boolean(runtimeHealth?.healthy);
}

function isHostReactivationBucket(bucket: RetryDecisionBucket): boolean {
...
function isHotLoopReason(reason?: string): boolean {
  return Boolean(
    reason && (REACTIVATE_REASON_CODES.has(reason) || RUNTIME_REASON_CODES.has(reason) || reason === "OUTCOME_NOT_CONFIRMED"),
  );
}

function getActiveReactivationCooldown(task: TaskRecord, reason?: string): string | undefined {
...
function shouldStartReactivationCooldown(task: TaskRecord, reason?: string): boolean {
  return isHotLoopReason(reason) && task.run_count >= HOT_LOOP_MIN_RUN_COUNT;
}

function shouldEscalateRepeatedFailureReview(task: TaskRecord, reason?: string): boolean {
  return Boolean(
    isHotLoopReason(reason) &&
      (task.reactivation_cooldown_count ?? 0) >= 1 &&
      task.run_count >= HOT_LOOP_ESCALATE_AFTER_COOLDOWN_RUN_COUNT,
  );
}

function hasStrongVisualSuccessSignal(hints: RetryDecisionArtifactHints): boolean {
...
function looksLikePaidReasonBoundary(text: string): boolean {
  return [
...
function inferReasonFromText(text: string, flowFamily?: TaskRecord["flow_family"]): string | undefined {
...
async function loadRetryDecisionArtifactHints(task: TaskRecord): Promise<RetryDecisionArtifactHints> {
  const candidatePaths = [...new Set([
    ...(task.latest_artifacts ?? []).slice().reverse(),
    task.wait?.evidence_ref,
  ].filter((value): value is string => typeof value === "string" && value.endsWith(".json")))]
    .sort((left, right) => scoreRetryDecisionArtifactPath(right) - scoreRetryDecisionArtifactPath(left));

  for (const candidatePath of candidatePaths) {
    const payload = await readJsonFile<Record<string, unknown>>(candidatePath);
    if (!payload) {
      continue;
    }

    const finalOutcome = payload.final_outcome as Record<string, unknown> | undefined;
    const proposedOutcome = payload.proposed_outcome as Record<string, unknown> | undefined;
    const earlyTerminalClassifier = payload.early_terminal_classifier as Record<string, unknown> | undefined;
    const visualVerification = payload.visual_verification as Record<string, unknown> | undefined;
    const finalWait = finalOutcome?.wait as Record<string, unknown> | undefined;
    const proposedWait = proposedOutcome?.wait as Record<string, unknown> | undefined;
    ...
    const waitReasonCode =
      (typeof finalWait?.wait_reason_code === "string" ? finalWait.wait_reason_code : undefined) ??
      (typeof proposedWait?.wait_reason_code === "string" ? proposedWait.wait_reason_code : undefined);

    const skipReasonCode =
      (typeof finalOutcome?.skip_reason_code === "string" ? finalOutcome.skip_reason_code : undefined) ??
      (typeof proposedOutcome?.skip_reason_code === "string" ? proposedOutcome.skip_reason_code : undefined);
    const classifierHypothesis =
      typeof earlyTerminalClassifier?.hypothesis === "string" ? earlyTerminalClassifier.hypothesis : undefined;
    const classifierRecommendedState =
      typeof earlyTerminalClassifier?.recommended_state === "string"
        ? (earlyTerminalClassifier.recommended_state as TaskStatus)
        : undefined;
    const classifierBusinessOutcome =
      typeof earlyTerminalClassifier?.recommended_business_outcome === "string"
        ? earlyTerminalClassifier.recommended_business_outcome
        : undefined;
    const classifierAllowRerun =
      typeof earlyTerminalClassifier?.allow_rerun === "boolean" ? earlyTerminalClassifier.allow_rerun : undefined;
    const visualClassification =
      typeof visualVerification?.classification === "string" ? visualVerification.classification : undefined;
    const visualConfidence =
      typeof visualVerification?.confidence === "number" ? visualVerification.confidence : undefined;
    const visualSummary =
      typeof visualVerification?.summary === "string" ? visualVerification.summary : undefined;
...
export async function buildRetryDecisionPlan(
  task: TaskRecord,
  runtimeHealth?: RetryRepartitionRuntimeHealth,
): Promise<RetryDecisionPlan> {
  ...
  if (
    classifierBusinessOutcome === "submitted_success" &&
  ...
  if (classifierBusinessOutcome === "blocked_policy" || classifierState === "WAITING_POLICY_DECISION") {
    const policyReasonCode =
      hints.classifierHypothesis === "reciprocal_backlink_required"
  ...
  if (
    inferredReason === "PAID_OR_SPONSORED_LISTING" ||
...
function applyRetryDecisionPlan(task: TaskRecord, plan: RetryDecisionPlan): boolean {
  let changed = false;
  ...
  const expectedWait =
    plan.nextStatus === "READY" || plan.nextStatus === "SKIPPED"
      ? undefined
      : {
  ...
  const expectedCooldownUntil =
    plan.waitReasonCode === REACTIVATION_COOLDOWN_WAIT_REASON_CODE ? plan.cooldownUntil : undefined;
  const expectedCooldownReason =
    plan.waitReasonCode === REACTIVATION_COOLDOWN_WAIT_REASON_CODE ? plan.cooldownReason : undefined;
  const startedNewCooldownWindow =
    plan.waitReasonCode === REACTIVATION_COOLDOWN_WAIT_REASON_CODE &&
    (task.reactivation_cooldown_until ?? undefined) !== expectedCooldownUntil;
  const expectedCooldownCount =
    plan.waitReasonCode === REACTIVATION_COOLDOWN_WAIT_REASON_CODE
...
export async function repartitionRetryDecisionTasks(args: {
  ...
  applyBuckets?: string[];
  maxApply?: number;
} = {}): Promise<RepartitionRetryDecisionResult> {
  const tasks = buildScopedTaskList(
    (await listTasks())
  ...
  const applyBuckets =
    parseBucketList(args.applyBuckets ?? (typeof args.maxApply === "number" ? DEFAULT_GUARDED_REACTIVATION_BUCKETS : undefined)) ??
    undefined;

  const byBucket: Record<string, number> = {};
  const plans: RetryDecisionPlan[] = [];
  let changed = 0;
  let applied = 0;
  ...
  const alreadyReleasedHosts: string[] = [];
...
export async function reapExpiredQueueState(): Promise<{
  reapedTaskId?: string;
  reapedTaskIds: string[];
  reapedBrowserOwnership: boolean;
}> {
...
export async function claimNextTask(args: {
  ...
  lease?: WorkerLease;
  reapedTaskId?: string;
  runtime_incident?: Awaited<ReturnType<typeof loadRuntimeIncident>>;
}> {
