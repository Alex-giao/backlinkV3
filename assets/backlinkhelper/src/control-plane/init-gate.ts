...
import type { InitGateMode, InitGateResult, MissingInputPreflightReport, PromotedProfile } from "../shared/types.js";
...
export async function loadPromotedProfileByScope(args: {
  promotedUrl?: string;
  promotedHostname?: string;
}): Promise<PromotedProfile | undefined> {
  const hostname = args.promotedUrl ? new URL(args.promotedUrl).hostname : args.promotedHostname;
  if (!hostname) {
    return undefined;
  }
  return readJsonFile<PromotedProfile>(getProfileFilePath(hostname));
}
