...
function scoutHasBloggerCommentFrame(scout: ScoutResult): boolean {
  return (scout.embed_hints ?? []).some((hint) => {
    ...
    const isBloggerCommentFrame =
      frameUrl.includes("blogger.com/comment/frame") || frameUrl.includes("draft.blogger.com/comment/frame");
    if (!isBloggerCommentFrame) {
      return false;
    }

    return (
      bodyText.includes("enter comment") ||
...
export function reclassifyTaskFlowFamilyFromScout(
  task: TaskRecord,
  scout: ScoutResult,
  reasonPrefix = "task-prepare scout classification",
): boolean {
...
function inferRegistrationRequired(task: TaskRecord, scoutTextHints: string[]): boolean {
  ...
  const visibleOauthOnly =
    (normalizedHints.has("with google") ||
...
function buildRegistrationDraft(task: TaskRecord): { account: AccountRecord; credentials: CredentialPayload } | undefined {
  if (!task.submission.submitter_email) {
    return undefined;
  }
...
function pickRecoveredTargetUrl(task: TaskRecord, scout: NonNullable<PrepareResult["scout"]>): string | undefined {
  ...
  const scored = (scout.link_candidates ?? [])
    .map((candidate) => {
      try {
        ...
        let score = 0;
        if (candidate.kind === "submit") score += 45;
        if (candidate.kind === "register") score += 20;
        if (candidate.kind === "auth") score += 5;
        const path = `${href.pathname}${href.search}`.toLowerCase();
        const text = candidate.text.toLowerCase();
        if (/(submit|add|listing|get-listed)/i.test(path)) score += 35;
        if (/(submit|add|listing|get listed)/i.test(text)) score += 20;
        if (/(sign-up|signup|register|join|create-account)/i.test(path)) score += 18;
        if (/(suggest|suggested)/i.test(path)) score -= 12;
        if (/\/categories\//i.test(path)) score -= 10;
        if (href.pathname !== "/") score += 4;
...
export function buildHomepageProbeUrl(targetUrl: string): string | undefined {
  const parsed = new URL(targetUrl);
  const homepageUrl = `${parsed.origin}/`;
  return parsed.href === homepageUrl ? undefined : homepageUrl;
}

export function shouldProbeHomepageForSubmitRecovery(
  task: TaskRecord,
  ...
  return (
    scout.page_assessment?.page_reachable === true &&
    (responseStatus === 404 ||
...
export function mustRunHomepageRecoveryBeforeRetry(args: {
  task: TaskRecord;
  scout: PrepareResult["scout"];
  homepageRecoveryAttempted: boolean;
}): boolean {
...
export function inferOpportunityClassFromScout(
  _task: TaskRecord,
  scout: PrepareResult["scout"],
): OpportunityClass {
  ...
  const hasExplicitSubmitSurface =
    (scout.submit_candidates?.length ?? 0) > 0 ||
    hasStructuredEntryCandidate ||
    hasInteractiveEmbed;

  if (
    hasExplicitSubmitSurface &&
    !ambiguityFlags.includes("not_found_but_reachable") &&
...
function scoutHasMixedSubmitAuthSignals(scout: ScoutResult): boolean {
  const ambiguityFlags = scout.page_assessment?.ambiguity_flags ?? [];
  return (
    ambiguityFlags.includes("mixed_submit_and_auth_signals") ||
    ambiguityFlags.includes("login_vs_register_ambiguous") ||
...
}): { wait_reason_code: string; detail: string } {
...
export async function prepareTaskForAgent(args: {
  ...
  try {
    ...
    if (!recoveredTargetUrl && homepageRecoveryRequired) {
      ...
      if (homepageProbeUrl) {
        const homepageProbeTask: TaskRecord = {
          ...task,
        ...
        task.notes.push(
          `Homepage probe for stale submit path ran against ${homepageProbeUrl}: ${homepageProbe.surface_summary}`,
        );
        ...
        if (recoveredTargetUrl) {
          ...
          task.notes.push(
            `Recovered submit route ${recoveredTargetUrl} from homepage probe after stale submit path on ${task.target_url}.`,
          );
        }
      }
    }
