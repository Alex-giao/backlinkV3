...
import type { UnattendedScopeTickResult } from "./unattended-scope-tick.js";
...
function makeLease(taskId: string): WorkerLease {
  return {
    task_id: taskId,
...
function makeReadyPrepare(task: TaskRecord): PrepareResult {
  return {
...
function makeStoppedPrepare(task: TaskRecord): PrepareResult {
  return {
    mode: "task_stopped",
...
function makeEnvelope(task: TaskRecord): AgentTraceEnvelope {
  return {
    trace: {
      task_id: task.id,
...
function makeFinalizeResult(): FinalizeResult {
  return {
    ok: true,
    next_status: "WAITING_SITE_RESPONSE",
    detail: "Submission accepted and waiting for publication.",
...
test("runUnattendedCampaign loops intake enqueue -> claim -> prepare -> operator -> record -> finalize -> follow-up", async () => {
  ...
  const calls: string[] = [];
  const scopeResults: UnattendedScopeTickResult[] = [
    {
      action: "enqueued",
      scope: { promotedHostname: "promo.example", promotedUrl: "https://promo.example/" },
  ...
  const result = await runUnattendedCampaign(
    ...
    },
    {
      ...
      recordAgentTrace: async (args: { taskId: string; envelope: AgentTraceEnvelope }) => {
      ...
      finalizeTask: async (args: { taskId: string }) => {
        calls.push(`finalize:${args.taskId}`);
        return makeFinalizeResult();
      },
      runFollowUpTick: async (args: { promotedUrl?: string }) => {
...
test("runUnattendedCampaign keeps pulling candidates after task-prepare stops a claimed task", async () => {
  ...
  const result = await runUnattendedCampaign(
    ...
    },
    {
      ...
      runOperator: async (context: { task: TaskRecord }) => {
        calls.push(`operator:${context.task.id}`);
